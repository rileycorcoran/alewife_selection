...
ohana_selscan <- fread(paste(in_prefix, "MAF001_geno01_scan_lle-ratios.txt", sep="_"), header=TRUE, sep="\t")[,-1]
if ( filtering %in% "original" ) {
   ohana_map <- fread(paste(in_prefix, "MAF001_geno01_hard_softfiltered.map", sep="_"), header=FALSE, sep="\t")[,-c(2:3)]
} else if ( filtering %in% "redone" ) {
   ohana_map <- fread(paste(in_prefix, "MAF001_geno01.map", sep="_"), header=FALSE, sep="\t")[,-c(2:3)]
}
ohana_pos <- cbind(ohana_selscan, ohana_map) %>%
  rename_with(~c("lle_ratio", "global_lle", "local_lle", "f_1", "f_2", "chr", "pos"))


## 50kb sliding window (by 10kb)
ohana_pos <- subset(ohana_pos, chr != "NC_014690.1")
...
fwrite(file=paste0(out_dir, "/pairwise_selscan_", lake, "_filtering_50kb_10kb_snpcount.txt"), x=ohana_windows, col.names=TRUE, row.names=FALSE, quote=FALSE, sep="\t")
